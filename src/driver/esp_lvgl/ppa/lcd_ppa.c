...
/* PPA (Pixel Processing Accelerator) is ESP32-P4 only. */
#ifdef ESP32P4_HW
...
static bool _lvgl_port_ppa_callback(ppa_client_handle_t ppa_client,
...
lvgl_port_ppa_handle_t lvgl_port_ppa_create(const lvgl_port_ppa_cfg_t *cfg) {
  ...
  ESP_GOTO_ON_FALSE(ppa_ctx, ESP_ERR_NO_MEM, err, TAG,
                    "Not enough memory for PPA context allocation!");
  ...
  ppa_ctx->buffer_size =
      ALIGN_UP(cfg->buffer_size, 64);  // PPA requires 64-byte alignment
  ppa_ctx->buffer = (uint8_t *)heap_caps_aligned_calloc(
      64, ppa_ctx->buffer_size, sizeof(uint8_t), buffer_caps);
  ...
  ESP_GOTO_ON_ERROR(
      ppa_register_client(&ppa_client_config, &ppa_ctx->srm_handle), err, TAG,
  ...
  ESP_GOTO_ON_ERROR(
      ppa_client_register_event_callbacks(ppa_ctx->srm_handle, &ppa_cbs), err,
      TAG, "Error when registering PPA callbacks!");
#endif
...
esp_err_t lvgl_port_ppa_rotate(lvgl_port_ppa_handle_t handle,
...
// Stub functions for non-PPA platforms
lvgl_port_ppa_handle_t lvgl_port_ppa_create(const lvgl_port_ppa_cfg_t *cfg) {
  return NULL;
}
void lvgl_port_ppa_delete(lvgl_port_ppa_handle_t handle) {}
